...
public class RenderSettingToggleButton<T extends Enum<T>> extends SettingToggleButton<T> {


    public RenderSettingToggleButton(Setting<T> setting, T val, IHandler<SettingToggleButton<T>> onPress) {
        super(setting, val, onPress);
    }
        ...
        if (this.visible) {
            ...
            if (this.isHalfSize()) {
                ...
                if (!isDisableBackground()) {
                    WirelessIcon.WIRELESS_BUTTON_BACKGROUND.getBlitter().dest(0, 0).blit(guiGraphics);
                }
